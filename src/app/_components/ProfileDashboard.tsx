    ...
    <div className="bg-gray-800/50 backdrop-blur-sm rounded-2xl p-6 md:p-10 border border-gray-700">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <div className="bg-gray-900/50 rounded-xl p-4">
          <p className="text-gray-400 text-xs mb-1">Topics Studied</p>
        ...
        <div className="bg-gray-900/50 rounded-xl p-4">
          <p className="text-gray-400 text-xs mb-1">Total Attempts</p>
          <p className="text-2xl font-bold text-white">{profile.performance.totalAttempts}</p>
        </div>
        <div className="bg-gray-900/50 rounded-xl p-4">
          <p className="text-gray-400 text-xs mb-1">Mastered</p>
      ...
      <div className="bg-purple-900/20 border border-purple-500/30 rounded-xl p-5 mb-6">
        <h3 className="font-semibold text-purple-300 mb-3">Your Learning Style</h3>
        <div className="space-y-2">
          {getLearningStyleDescription().map((style, i) => (
            <p key={i} className="text-gray-300 text-sm">{style}</p>
          ))}
        </div>
        <div className="mt-4 grid grid-cols-3 gap-3">
          <div>
            <p className="text-xs text-gray-500 mb-1">Analogies</p>
      ...
      <div className="space-y-4 mb-6">
        ...
        {profile.topicsCovered.length === 0 && (
          <div className="bg-gray-900/30 rounded-xl p-8 text-center">
            <p className="text-gray-400">No topics studied yet. Start learning to build your profile!</p>
          </div>
        )}
      </div>
      ...
      <div className="bg-gray-900/30 rounded-xl p-4 mb-6">
        <div className="grid grid-cols-2 gap-4 text-sm">
          <div>
            ...
            <p className="text-gray-300">{formatDate(profile.createdAt)}</p>
          </div>
          <div>
            <p className="text-gray-500">Last Active</p>
            <p className="text-gray-300">{formatDate(profile.lastActive)}</p>
          </div>
        </div>
      </div>
